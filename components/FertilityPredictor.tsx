...
interface FertilityPrediction {
  date: string;
  type: 'fertile_window' | 'ovulation' | 'period' | 'low_fertility';
...
interface FertilityPredictorProps {
...
export default function FertilityPredictor({ 
  cycleHistory, 
  currentCycleData, 
  onLogData, 
  onViewInsights 
}: FertilityPredictorProps) {
  const [predictions, setPredictions] = useState<FertilityPrediction[]>([]);
  ...
  const generatePredictions = () => {
    ...
    const newPredictions: FertilityPrediction[] = [];
  ...
  const renderPredictionDetails = () => {
    if (!selectedDate || !showPredictionDetails) return null;
    ...
    return (
      <View style={styles.predictionDetailsCard}>
        <View style={styles.predictionHeader}>
          <View style={styles.predictionIconContainer}>
            {getPredictionIcon()}
          </View>
          <View style={styles.predictionInfo}>
            <Text style={styles.predictionTitle}>{getPredictionTitle()}</Text>
      ...
      <View style={styles.chartContainer}>
        ...
        <View style={styles.chartContent}>
          <Svg width="100%" height={120} viewBox="0 0 300 120">
            {next7Days.map((prediction, index) => {
              const x = (index / 6) * 280 + 10;
              const y = 100 - (prediction.probability / 100) * 80;
              const color = prediction.type === 'fertile_window' ? colors.reproductive.ovaries :
                           prediction.type === 'ovulation' ? colors.semantic.healing :
                           prediction.type === 'period' ? colors.reproductive.uterus :
            ...
            <Path
              d={next7Days.map((prediction, index) => {
                const x = (index / 6) * 280 + 10;
                const y = 100 - (prediction.probability / 100) * 80;
        ...
        <View style={styles.chartLegend}>
          {next7Days.map((prediction, index) => (
            <View key={index} style={styles.legendItem}>
              ...
              <Text style={styles.legendProbability}>{prediction.probability}%</Text>
            </View>
          ))}
        </View>
      </View>
  ...
  const getTodayPrediction = () => {
    const today = new Date().toISOString().split('T')[0];
    return predictions.find(p => p.date === today);
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        <Text style={styles.title}>Fertility Predictor</Text>
        <Text style={styles.subtitle}>AI-powered cycle analysis and predictions</Text>
      </View>
        ...
        <View style={styles.todayStatusCard}>
          <View style={styles.todayStatusHeader}>
            <View style={styles.todayIcon}>
              {todayPrediction.type === 'fertile_window' ? <Heart size={24} color={colors.nude.background} /> :
               todayPrediction.type === 'ovulation' ? <Zap size={24} color={colors.nude.background} /> :
               todayPrediction.type === 'period' ? <Droplets size={24} color={colors.nude.background} /> :
            ...
            <View style={styles.todayInfo}>
              ...
              <Text style={styles.todayStatus}>
                {todayPrediction.type === 'fertile_window' ? 'Fertile Window' :
                 todayPrediction.type === 'ovulation' ? 'Ovulation Day' :
                 todayPrediction.type === 'period' ? 'Period Day' :
                 'Low Fertility'}
              </Text>
              <Text style={styles.todayProbability}>
                {todayPrediction.probability}% probability
              </Text>
            </View>
          </View>
        </View>
      ...
      <View style={styles.actionButtons}>
        ...
        >
          <Activity size={20} color={colors.nude.roseGold} />
